...
export const ApplicationList = (props: {
    ...
    const getApplications = () => {
        ApiService.getApplications().then((value: ApiResponse<Application>) => {
            console.log(value)
            if (value.data && value.data.data?.entity && Array.isArray(value.data.data.entity)) {
                setApplications(value.data.data.entity);
            }
        });
    };
                ...
                <Table style={{minWidth: 250}} aria-label="simple table">
                    ...
                    <TableBody>
                        {applications.map((app: Application, idx: number) => (
                            <TableRow
                                key={idx}
                                sx={{"&:last-child td, &:last-child th": {border: 0}, "td, th": {padding: 0.2}}}
                            >
                                <TableCell
                                    component="th"
                                    scope="row"
